...
module.exports = {
  structure: {
    name: "level-extraxp-channel",
    description: "add or remove a extraxp channel from leveling system",
    aliases: ["l-extraxp-channel", "lextraxp-channel", "levelextraxp-channel"],
      ...
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle("Leveling")
            .setDescription(
              "Please provide a valid type\n\n**Example:**\n```level-extraxp-channel add #channel```"
            )
            .setColor(client.color),
        ],
      });
...
if(channel.type !== 0 && channel.type !== 5) return message.reply({
    ...
    if (type === "add") {
      ...
      await data.update({ ExtraXP: [...data.ExtraXP, channel.id] });
      message.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle("Leveling")
            .setDescription(`Channel ${channel} has been added to extraxp`)
            .setColor(client.color),
        ],
      });
    } else if (type === "remove") {
        ...
        return message.reply({
          embeds: [
            new EmbedBuilder()
              .setTitle("Leveling")
              .setDescription(
                "This channel is not added to  extra xp channels so it can't be removed from it"
              )
              .setColor(client.color),
          ],
        });
      await data.update({
        ExtraXP: data.ExtraXP.filter((ch) => ch !== channel.id),
      });
      message.reply({
        embeds: [
          new EmbedBuilder()
            .setTitle("Leveling")
            .setDescription(
              `Channel ${channel} has been removed from extraxp channels`
            )
            .setColor(client.color),
        ],
      });

...
module.exports = {
  structure: new SlashCommandBuilder()
    .setName("level-announce-embed")
    .setDescription("set Level Announce Embed ")
  ...
  run: async (client, interaction) => {
    ...
    let embed = new EmbedBuilder()
      ...
      .setThumbnail(
        "https://cdn.discordapp.com/avatars/1051806381461745664/f39828e0fc358084e82fb9582bef1fc1.png"
      )
      .setTimestamp();

    const msg = await interaction.editReply({
      embeds: [embed],
      components: [row, row2, rowb],
    });

    const VariableEmbed = interaction.channel.send({
      embeds: [
        new EmbedBuilder()
          .setDescription(
            `
...
{guild:id} - Guild ID
    ...
    const filter = (i) => i.user.id === interaction.user.is;
    ...
    collector.on("collect", async (i) => {
      if (i.customId === "embedSelect") {
        ...
        interaction.channel
          .send({
            content:
              "Please Send Embed Format Of Aayu Which You Copied From [Here](https://www.aayubot.me/embed-visualizer.html)",
          })
          .then((message2) => {
            const filterMessage = (m) =>
              m.author.id === message2.author.id && !m.author.bot;
            message2.channel
              ...
              .then(async (collected) => {
                ...
                if (!embed)
                  return (
                    message.channel.send({ content: "Invalid Embed Format" }),
                    console.error("Error building embed:", { success, fails })
                  );

                msg.edit({ embeds: [embed] }).then(() => {
                  i.followUp({
                    content: "Embed Has Been Set",
                    ephemeral: true,
                  });
                });
        ...
        interaction.channel
          .send({
            content: `Paste A Json Here or You Can Send a raw Json Link of Embed You Can USe [Github](https://github.com), [Pastebin](https://pastebin.com) or any other site that gives json. You Can Build Json On [Here](https://www.aayubot.me/embed-visualizer.html) . You can Also Send Attachment of file`,
          })
          .then((message2) => {
            const filterMessage = (m) =>
              m.author.id === message2.author.id && !m.author.bot;
            message2.channel
              ...
              .then(async (collected) => {
                ...
                if (
                  ...
                  if (!data)
                    return interaction.channel.send({
                      content: "Invalid Json Attachment",
                    });
                  ...
                  try {
                    ...
                    return interaction.channel.send({
                      content: "Invalid Json Text Format",
                    });
                  }
                }
              ...
              .then(async (collected) => {
                ...
                if (content.toLowerCase() == "guildavatar") {
                  ...
                  i.followUp({
                    embeds: [
                      new EmbedBuilder()
                        .setTitle("Leveling")
                        .setDescription(
                          "Thumbnail Has Been Set To Guild Avatar.  Now It Will Show On The Level Up Embed"
                        )
                        .setColor(client.color),
                    ],
                    ephemeral: true,
                  ...
                  i.followUp({
                    embeds: [
                      new EmbedBuilder()
                        .setTitle("Leveling")
                        .setDescription(
                          "Thumbnail Has Been Set To Author Avatar. Now It Will Show On The Level Up Embed"
                        )
                        .setColor(client.color),
                    ],
                    ephemeral: true,
                  ...
                  i.followUp({
                    embeds: [
                      new EmbedBuilder()
                        .setTitle("Leveling")
                        .setDescription(
                          "Thumbnail Setting Has Been Set To None. Now You Can Set Your Custom Thumnail URL"
                        )
                        .setColor(client.color),
                    ],
                    ephemeral: true,
                  ...
                  interaction.channel.send({
                    content:
                      "Invalid Parameters. The Parameters Should Be   `None`, `AuthorAvatar` or `guildAvatar` ",
                  });
                }
              });
          });
